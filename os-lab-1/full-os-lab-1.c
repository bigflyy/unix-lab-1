...
void* provider_thread(void* arg) {
    ...
    while(1) { // В реальном коде обычно добавляется условие остановки
        ...
        // критическая секция - участок кода, в котором поток работает с разделяемыми ресурсами 
...
int main() {
    ...
    // Создаем потоки
    // место куда будет записан ID нового потока
    // NULL - использовать стандартные аттрибуты потока (размер стека, адрес, приоритет)
    // функция, с которой начнет выполнение новый поток
    // NULL - в качестве аргумента функции provider_thread  
